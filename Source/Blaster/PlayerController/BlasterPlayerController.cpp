...
void ABlasterPlayerController::ServerRequestServerTime_Implementation(float TimeOfClientRequest)
{
    ...
    //Calling Server to client function 
    ClientReportServerTime(TimeOfClientRequest, ServerTimeOfReceipt);
}

void ABlasterPlayerController::ClientReportServerTime_Implementation(float TimeOfClientRequest, float TimeServerReceivedClientRequest)
{
    //difference of time when client sending server rpc
    float RoundTripTime = GetWorld()->GetTimeSeconds() - TimeOfClientRequest;
    SingleTripTime = 0.5f * RoundTripTime;
    //Calculating servers current time 
    float CurrentServerTime = TimeServerReceivedClientRequest + SingleTripTime;

    //Value that gets displayed on server and client
...
void ABlasterPlayerController::SetHUDMatchCountdown(float CountdownTime)
{
    ...
    if (bHUDValid)
    {
        ...
        //Changing color with 30 seconds left
        if (CountdownTime < 30.f)
        {
           
            const FSlateColor CountdownColor(FLinearColor::Red);
